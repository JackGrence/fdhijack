...
static int fdhj_exchangefd(pid_t pid1, int fd1, pid_t pid2, int fd2)
{
	...
	rcu_read_lock();
	struct task_struct *task1 = get_pid_task(find_get_pid(pid1), PIDTYPE_PID);
	struct task_struct *task2 = get_pid_task(find_get_pid(pid2), PIDTYPE_PID); 
	rcu_read_unlock();
	if (task1 == NULL) {
